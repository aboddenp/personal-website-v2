...
// Deferred so radix + motion + sounds stay out of the initial bundle; fetched on first open
const Dialog = dynamic(() => import('@/components/Dialog'), { ssr: false });

function Projects() {
  const { openDialog, isOpen } = useDialog();
  // Mount on first open and keep mounted so the exit animation isn't cut off
      ...
      {dialogMounted && (
        <Dialog description={'Dialog Modal exploring what is Atlas Theme'} title="Atlas Overview">
          <section style={{ maxWidth: '80ch', margin: '0 auto' }}>
            ...
            <p>
              I maintained <strong>Atlas Free</strong> and <strong>Atlas Pro</strong>, two of the most popular HubSpot
              CMS themes for B2B SaaS. My role involved debugging user issues, implementing feature requests, and
              shipping updates to improve flexibility and maintainability.
            </p>
            <p>
              As usage grew, so did the friction. Our team decided to rebuild from the ground up—
              <strong>Atlas 2</strong>
              —with a focus on developer ergonomics and marketing usability. Instead of dozens of rigid modules, we
              built a smaller set of <strong>highly configurable components</strong> with smart defaults and custom
              styling options.
            </p>
            <p>
